...
import { Cpu, Mail, Lock, Eye, EyeOff, ChevronRight, Zap, Shield, Star, ArrowRight, Monitor, Package } from 'lucide-react';
    ...
    <>
      ...
      <div className="lp-bg">
        ...
        {[
          ...
          <div key={i} className="lp-float" style={s as React.CSSProperties}>{e}</div>
        ))}
      </div>
      ...
      <div className="lp-root lp-content">
        ...
        <div className="lp-left">
          ...
          {/* Recent activity */}
          <div>
            <div style={{ color:'rgba(148,163,184,.6)',fontSize:11,fontWeight:600,letterSpacing:'.8px',textTransform:'uppercase',marginBottom:10 }}>
              What awaits you
            </div>
            <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
              {[
                { icon:'📦', label:'Track your orders', sub:'Real-time delivery updates' },
                { icon:'❤️', label:'Your wishlist', sub:'Saved items ready to buy' },
                { icon:'🏷️', label:'Member deals', sub:'Exclusive discounts for you' },
                { icon:'🔧', label:'PC Builder', sub:'Your saved configurations' },
              ].map(({ icon, label, sub }) => (
                <div key={label} className="lp-activity">
                  <span style={{ fontSize:18 }}>{icon}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ color:'#fff',fontSize:12.5,fontWeight:600 }}>{label}</div>
                    <div style={{ color:'rgba(148,163,184,.7)',fontSize:11,marginTop:1 }}>{sub}</div>
                  </div>
        ...
        <div className="lp-right">
          <div className="lp-card">
            ...
            {/* Logo */}
            <div className="lp-row" style={{ display:'flex',alignItems:'center',gap:10,marginBottom:24 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="lp-row" style={{ marginBottom:14 }}>
                <label htmlFor="email" style={{ display:'block',fontSize:13,fontWeight:600,color:'#374151',marginBottom:5 }}>Email</label>
                <div style={{ position:'relative' }}>
                  <Mail size={15} color={focused==='email'?'#3b82f6':'#94a3b8'}
              ...
              <div className="lp-row" style={{ marginBottom:8 }}>
                <label htmlFor="password" style={{ display:'block',fontSize:13,fontWeight:600,color:'#374151',marginBottom:5 }}>Password</label>
                <div style={{ position:'relative' }}>
                  <Lock size={15} color={focused==='password'?'#3b82f6':'#94a3b8'}
                    style={{ position:'absolute',left:13,top:'50%',transform:'translateY(-50%)',transition:'color .25s',pointerEvents:'none' }}/>
                  <input id="password" type={showPw?'text':'password'} placeholder="••••••••" value={password}
                    onChange={e => setPassword(e.target.value)}
                    onFocus={() => setFocused('password')} onBlur={() => setFocused('')}
                    required className="lp-input" style={{ paddingRight:42 }}/>
              ...
              {/* Remember me + forgot */}
              <div className="lp-row" style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20 }}>
                <label style={{ display:'flex',alignItems:'center',gap:8,cursor:'pointer' }}>
                  <input type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)}
                    style={{ accentColor:'#3b82f6',width:15,height:15,cursor:'pointer' }}/>
                  <span style={{ fontSize:13,color:'#374151',fontWeight:500 }}>Remember me</span>
                </label>
                <a href="#" style={{ fontSize:13,color:'#3b82f6',textDecoration:'none',fontWeight:500,transition:'opacity .2s' }}
            ...
            {/* Divider */}
            <div className="lp-row lp-divider" style={{ color:'#94a3b8',fontSize:12 }}>or continue with</div>

            {/* Social buttons */}
            <div className="lp-row" style={{ display:'flex',gap:10,marginBottom:4 }}>
            ...
            {/* Sign up link */}
            <div className="lp-row" style={{ textAlign:'center',marginTop:20,paddingTop:18,borderTop:'1px solid #f1f5f9' }}>
              <span style={{ color:'#64748b',fontSize:14 }}>Don't have an account? </span>
              <Link to="/signup" style={{ color:'#3b82f6',fontWeight:600,fontSize:14,textDecoration:'none' }}>Sign up →</Link>
            </div>

            <div className="lp-row" style={{ textAlign:'center',marginTop:10 }}>
              <span style={{ fontSize:11,color:'#94a3b8' }}>🔒 Secured with 256-bit SSL encryption</span>
            </div>
          </div>
        </div>
      </div>
